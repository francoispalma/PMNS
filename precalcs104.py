...
def do_precalcs(p, n, gamma, lam):
	...
	# Then we print
	print("static const uint64_t Mlo[] = {" + str([elem % (2**52) for elem in M])[1:-1].replace(",", "u,") + "u},")
	print("\tM1lo[] = {" + str([elem % (2**52) for elem in M1])[1:-1].replace(",", "u,") + "u},")
	print("\tMLambdalo[] = {" + str([(elem * lam) % 2**52 for elem in M])[1:-1].replace(",", "u,") + "u},")
	print("\tM1Lambdalo[] = {" + str([elem % 2**52 for elem in M1L])[1:-1].replace(",", "u,") + "u},")
	print("\t__Pilo__[N][N] = {")
	for i in range(len(Pi) - 1):
		print("\t\t{" + str([elem % 2**52 for elem in Pi[i]])[1:-1].replace(",", "u,") + "u},")
	print("\t\t{" + str([elem % 2**52 for elem in Pi[-1]])[1:-1].replace(",", "u,") + "u}\n\t};\n")

	print("static const int64_t Mhi[] = {" + str([elem >> 52 for elem in M])[1:-1] + "},")
	print("\tM1hi[] = {" + str([elem >> 52 for elem in M1])[1:-1] + "},")
	print("\tMLambdahi[] = {" + str([(elem * lam) >> 52 for elem in M])[1:-1] + "},")
	print("\tM1Lambdahi[] = {" + str([elem >> 52 for elem in M1L])[1:-1] + "},")
	print("\t__Pihi__[N][N] = {")
	for i in range(len(Pi) - 1):
		print("\t\t{" + str([elem >> 52 for elem in Pi[i]])[1:-1] + "},")
	print("\t\t{" + str([elem >> 52 for elem in Pi[-1]])[1:-1] + "}\n\t};\n")
