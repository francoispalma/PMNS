...
const mpnum __P1024__ = &__P__;
const mpnum Gamma1024 = Gi;
const int8_t N1024 = N;
const int8_t RHO1024 = RHO;
...
static void Mtoeplitz_vm_10x10(__int128* restrict rop, const int64_t* restrict vect, const int64_t* restrict matr)
{
	#pragma GCC unroll 10
	for(int i = 0; i < 10; i++)
	{
		...
		#pragma GCC unroll 10
		for(int j = 0; j < 10; j++)
			rop[i] += (__int128) vect[j] * matr[10 - 1 - j + i];
	}
}

static void Mtoeplitz_vm20x20(__int128* restrict rop, const int64_t* restrict vect)
{
	...
	for(i = 0; i < 19; i++)
	{
		m0m1[i] = matrM[i + 10] - matrM[i + 20];
		m0m2[i] = matrM[i + 10] - matrM[i];
	}
	
	Movtoeplitz_vm_10x10(t0, v0p1, matrM + 10);
	Mtoeplitz_vm_10x10(t1, vect, m0m1);
	Mtoeplitz_vm_10x10(t2, vect + 10, m0m2);
...
static void M1toeplitz_vm20x20(int64_t* restrict rop, const __int128* restrict vect)
{
	int64_t t0[10], t1[10], t2[10];
	int64_t v0[10], v1[10], v0p1[10], m0m1[19], m0m2[19];
	register uint16_t i;
	...
	for(i = 0; i < 19; i++)
	{
		m0m1[i] = matrM1[i + 10] - matrM1[i + 20];
		m0m2[i] = matrM1[i + 10] - matrM1[i];
	}
	
	M1toeplitz_vm_10x10(t0, v0p1, matrM1 + 10);
	M1toeplitz_vm_10x10(t1, v0, m0m1);
	M1toeplitz_vm_10x10(t2, v1, m0m2);
	...
	const restrict poly A, const restrict poly B)
{
	...
	toeplitz_vm_20x20(R, A->t, matrix);
}
...
static inline void pmns_montg_int_red(restrict poly res, __int128* restrict R)
{
	...
	M1toeplitz_vm20x20(T, R);
	
	Mtoeplitz_vm20x20(R, T);
