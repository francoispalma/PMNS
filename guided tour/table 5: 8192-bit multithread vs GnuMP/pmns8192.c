...
static inline void pmns_montg_int_red(restrict poly res, __int128* restrict R, const uint8_t NBTHREADZ)
{
	// Internal reduction of R via the Polynomial Montgomery-like method.
	int64_t T[N];
	register uint16_t i;
	
	b1_pmns_mult(T, R, NBTHREADZ);
	
	b_pmns_mult(R, T, NBTHREADZ);
	...
	const restrict poly B, const uint8_t NBTHREADZ)
{
	...
	pmns_mod_mult_ext_red(R, A, B, NBTHREADZ);
	
	pmns_montg_int_red(res, R, NBTHREADZ);
}
