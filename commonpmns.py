...
_filelist.remove("primes7168.py")

_primesizes = [name[6:-3] for name in _filelist if "primes" in name]

primesdict = {int(prime): getattr(importlib.import_module("generated." + filename[:-3]), "PRIMES" + prime) for filename in _filelist for prime in _primesizes if "primes" + prime + ".py" in filename}
